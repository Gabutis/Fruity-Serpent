...
def game_state_menu(auto_move):
    ...
    leaderboard_button_border = menu_font.render("Press L or click for Leaderboard", True, settings.GREEN)
    leaderboard_button = menu_font.render("Press L or click for Leaderboard", True, settings.PURPLE)
    ...
    screen.blit(start_button_border, (settings.WIDTH // 2 - start_button.get_width() // 2 + 2, settings.HEIGHT // 2 - 50))
    screen.blit(start_button_border, (settings.WIDTH // 2 - start_button.get_width() // 2, settings.HEIGHT // 2 - 2 -50))
    screen.blit(start_button_border, (settings.WIDTH // 2 - start_button.get_width() // 2, settings.HEIGHT // 2 + 2 -50))
    screen.blit(start_button, (settings.WIDTH // 2 - start_button.get_width() // 2, settings.HEIGHT // 2 - 50))

    screen.blit(leaderboard_button_border, (settings.WIDTH // 2 - leaderboard_button.get_width() // 2 - 2, settings.HEIGHT // 2))
    screen.blit(leaderboard_button_border, (settings.WIDTH // 2 - leaderboard_button.get_width() // 2 + 2, settings.HEIGHT // 2))
    screen.blit(leaderboard_button_border, (settings.WIDTH // 2 - leaderboard_button.get_width() // 2, settings.HEIGHT // 2 - 2))
    screen.blit(leaderboard_button_border, (settings.WIDTH // 2 - leaderboard_button.get_width() // 2, settings.HEIGHT // 2 + 2))
    screen.blit(leaderboard_button, (settings.WIDTH // 2 - leaderboard_button.get_width() // 2, settings.HEIGHT // 2))

    screen.blit(exit_button_border, (settings.WIDTH // 2 - exit_button.get_width() // 2 - 2, settings.HEIGHT - 50))
    screen.blit(exit_button_border, (settings.WIDTH // 2 - exit_button.get_width() // 2 + 2, settings.HEIGHT - 50))
    screen.blit(exit_button_border, (settings.WIDTH // 2 - exit_button.get_width() // 2, settings.HEIGHT - 50 - 2))
    screen.blit(exit_button_border, (settings.WIDTH // 2 - exit_button.get_width() // 2, settings.HEIGHT - 50 + 2))
    screen.blit(exit_button, (settings.WIDTH // 2 - exit_button.get_width() // 2, settings.HEIGHT - 50))
...
def game_state_name_input(player_name):
    ...
    input_text = input_font.render("Enter your name and press SPACE or click to start:", True, settings.PURPLE)
    input_text_position = (settings.WIDTH // 2 - input_text.get_width() // 2, settings.HEIGHT // 2 - 50)
    ...
    player_name_text_position = (settings.WIDTH // 2 - player_name_text.get_width() // 2, settings.HEIGHT // 2)
...
def game_state_game(player_name, score, auto_move):
    ...
    screen.blit(score_text_border, (settings.GRID_SIZE / 2 - 2, settings.GRID_SIZE / 2 - 2))
    screen.blit(score_text_border, (settings.GRID_SIZE / 2 + 2, settings.GRID_SIZE / 2 - 2))
    screen.blit(score_text_border, (settings.GRID_SIZE / 2 - 2, settings.GRID_SIZE / 2 + 2))
    screen.blit(score_text_border, (settings.GRID_SIZE / 2 + 2, settings.GRID_SIZE / 2 + 2))
    ...
    if player_name and not auto_move:
        screen.blit(name_text_border, (settings.GRID_SIZE * 8, settings.GRID_SIZE / 2 - 2))
        screen.blit(name_text_border, (settings.GRID_SIZE * 8 + 2, settings.GRID_SIZE / 2 - 2))
        screen.blit(name_text_border, (settings.GRID_SIZE * 8 - 2, settings.GRID_SIZE / 2 + 2))
        screen.blit(name_text_border, (settings.GRID_SIZE * 8 + 2, settings.GRID_SIZE / 2 + 2))
...
def game_state_leaderboard(leaderboard):
    ...
    sorted_scores = sorted(leaderboard.scores, key=lambda entry: entry['score'], reverse=True)
    ...
    for i, entry in enumerate(sorted_scores):
        ...
        entry_rect = entry_text_border.get_rect(midtop=(settings.WIDTH // 2, 100 + i * 30))
